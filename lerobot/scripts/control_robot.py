...
def record_with_bbox(
    ...
    while True:
        ...
        log_say(f"Object detection using Gemini", cfg.play_sounds)
        start_episode_t = time.perf_counter()
        while True:
            observation, _ = robot.teleop_step(record_data=True)  
            if time.perf_counter() - start_episode_t > 2:
                break
        prompt = "Detect the 2d bounding boxes of bottle and steel-glass. Not the paper cup" 
...
def control_robot(cfg: ControlPipelineConfig):
    ...
    elif isinstance(cfg.control, RecordWithBboxControlConfig):
        _init_rerun(control_config=cfg.control, session_name="Aditya_control_loop_record_with_bbox")
        record_with_bbox(robot, cfg.control)
